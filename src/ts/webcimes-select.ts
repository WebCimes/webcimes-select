...
class WebcimesSelectImpl implements WebcimesSelect {
  ...
  private getHtmlElements(element: string | HTMLElement | NodeList | null) {
    ...
    if (typeof element === "string") {
      htmlElements = [
        ...(Array.from(document.querySelectorAll(element)) as HTMLElement[]),
      ];
    }
    return htmlElements;
  }
  ...
  private getUniqueID(
    ...
    do {
      ...
      element.querySelector(selectorPrefix + identifier + selectorSuffix)
    );
  ...
  private init() {
    ...
    this.nativeSelect = this.getHtmlElement(
    ...
    if (this.nativeSelect) {
      ...
      this.idDropdownOptions = this.getUniqueID(
        "#",
        "webcimes-dropdown-options-"
      );
  ...
  public removeAllOptions() {
    if (!this.nativeSelect?.disabled) {
      ...
      if (this.options.keepOpenDropdown) {
        ...
        this.dropdown
          ?.querySelectorAll(`.webcimes-dropdown__option`)
          .forEach((el) => {
  ...
  private onKeyDown(e: KeyboardEvent) {
    if (
      (e.target as HTMLElement).closest(
        ".webcimes-select > .webcimes-select__clear"
      )
    ) {
      if (e.key == "Enter") {
        e.preventDefault();
      ...
      (e.target as HTMLElement).closest(
        ".webcimes-select__option .webcimes-select__clear"
      )
    ) {
      if (e.key == "Enter") {
        e.preventDefault();
  ...
  private onClickInitDropdown(e: Event) {
    ...
    if (
      !this.dropdown &&
      !(e.target as HTMLElement).closest(".webcimes-select__clear")
    ) {
      this.initDropdown();
    }
    // Close dropdown
          ...
          this.nativeSelect!.getAttribute("dir") == "rtl" ? `dir="rtl"` : ``
            ...
            this.options.allowSearch
              ...
              : ``
					...
					<div class="webcimes-dropdown__options" id="${
            this.idDropdownOptions
          }" role="listbox" ${
  ...
  private setDropdownOptions(options: HTMLOptionElement[]) {
    ...
    this.dropdown!.querySelector(
      ".webcimes-dropdown__options"
    )!.replaceChildren(optionsEl.content);
  ...
  private setDropdownHighlightOption(index: number, autoScroll: boolean) {
    ...
    highlightedOption?.classList.remove(
      "webcimes-dropdown__option--highlighted"
    );
    ...
    if (optionsEl[index]) {
      ...
      this.dropdown!.querySelector(
        ".webcimes-dropdown__search-input"
      )?.setAttribute("aria-activedescendant", highlightedOption.id);
      if (autoScroll) {
  ...
  private onDropdownMouseOverOption(e: MouseEvent) {
    let optionEl = (e.target as HTMLElement).closest(
    ...
    if (optionEl) {
      ...
      this.setDropdownHighlightOption(
        Array.from(optionsEl).indexOf(optionEl),
        false
      );
    }
  }
 ...
 */
export function WebcimesSelect(options: Partial<Options>): WebcimesSelect {
